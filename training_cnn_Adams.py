...
from cnn_model import Dense, ReLU, Flatten, softmax, categorical_crossentropy, crossentropy_backward, AdamOptimizer, step_decay
...
def train_network(X_train, Y_train, X_val, Y_val, epochs, batch_size, run_config):
    """Training loop with validation, accuracy tracking, and CSV logging."""
    
    # Determine input size and class count
    input_dim = X_train.shape[1]
    num_classes = Y_train.shape[1]

    # FIXED: Proper architecture flow (Input -> 128 -> 64 -> num_classes)
    # Removed ReLU after the final Dense layer
    network = [
        Flatten(),
    ...
    # FIXED: Collect params in a specific, repeatable order
    trainable_params = []
    for layer in network:
        if isinstance(layer, Dense):
            trainable_params.append(layer.weights)
            trainable_params.append(layer.biases)

        # Inside train_network()
        optimizer = AdamOptimizer(trainable_params, lr=run_config['initial_lr'])
    ...
    # FIXED: Initialize all required history keys
    history = {
        'epoch':      [],
        'train_loss': [],
        'val_loss':   [],
    ...
    csv_file = 'CNN_training_logs.csv'
    with open(csv_file, 'w', newline='') as f:
        csv.writer(f).writerow(['epoch', 'train_loss', 'val_loss', 'train_acc', 'val_acc', 'learning_rate'])

    print(f"Starting training for {epochs} epochs...")
    print(f"Architecture: Flatten → Dense({input_dim}, 128) → ReLU → Dense(128, 64) → ReLU → Dense(64, {num_classes})\n")
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(X_train), batch_size):
            ...
            # Backward Pass
            # FIXED: Maintain explicit mapping for gradients to avoid index confusion
            grads_dict = {}
            for layer in reversed(network):
                if isinstance(layer, Dense):
                    grad, dW, db = layer.backward(grad)
                    grads_dict[id(layer)] = (dW, db)
                else:
                    grad = layer.backward(grad)

            # FIXED: Reconstruct gradient list in EXACT order of trainable_params
            layer_gradients = []
            for layer in network:
                if isinstance(layer, Dense):
                    dW, db = grads_dict[id(layer)]
                    layer_gradients.append(dW)
                    layer_gradients.append(db)
        ...
        # ── Epoch metrics (FIXED: Complete the missing implementation) ────────────────────────────────
        ...
        # FIXED: Store complete history for plotting
        ...
        with open(csv_file, 'a', newline='') as f:
            csv.writer(f).writerow([epoch + 1, train_loss, val_loss, train_acc, val_acc, optimizer.lr])

        # Print progress
        print(f"Epoch {epoch+1:02d}/{epochs}  "
              f"loss={train_loss:.4f}  acc={train_acc:.4f}  "
              f"val_loss={val_loss:.4f}  val_acc={val_acc:.4f}  lr={optimizer.lr:.6f}")
        ...
        if val_loss < best_val_loss:
            ...
            checkpoint = {
                'epoch':           epoch + 1,
                'optimizer_state': {'t': optimizer.t, 'lr': optimizer.lr},
                'run_config':      run_config,
                'weights':         [p.tolist() for p in trainable_params]
            }
            with open('CNN_best_checkpoint.json', 'w') as f:
...
def plot_results(history, cm, precision, recall, f1,
    ...
    fig.suptitle('CNN Results', fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    out_path = os.path.join(output_dir, 'cnn_results.png')
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'l2_lambda':    0.01,
        'clip_value':   1.0,
        'architecture': ['Flatten', 'Dense(input, 128)', 'ReLU', 'Dense(128, 64)', 'ReLU', f'Dense(64, {num_classes})']
    }

    # 4. Train
    print(f"\nDataset sizes: Train={len(X_train)}, Val={len(X_val)}, Test={len(X_test)}")
    print(f"Input features: {X_train.shape[1]}, Num classes: {num_classes}\n")
    ...
    # 8. Save text report
    with open('CNN_evaluation_report.txt', 'w') as f:
